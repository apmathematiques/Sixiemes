(function(global){
  ...
  const Random = {
    ...
    line({
      ...
      return new Line(
        {
          x:Random.number(margin,width-margin),
          y:Random.number(margin,height-margin)
        },
        Random.number(minAngle,maxAngle)
      );
    },

    reachableAreaForTool(tool,{
      width=900,
      height=330,
      extraMargin=18
    }={}){
      if(tool instanceof SetSquare || tool==="setSquare"){
        const toolWidth=tool instanceof SetSquare ? tool.width : 180;
        const toolHeight=tool instanceof SetSquare ? tool.height : 130;

        /*
          Le sommet de l’angle droit doit rester assez loin des bords pour
          que l’équerre puisse être placée dans plusieurs orientations.
          On utilise un rayon de rotation prudent fondé sur la diagonale.
        */
        const radius=Math.hypot(toolWidth,toolHeight);
        const horizontal=Math.min(width/2-10,Math.max(toolWidth*.72,radius*.52)+extraMargin);
        const vertical=Math.min(height/2-10,Math.max(toolHeight*.72,radius*.42)+extraMargin);

        return {
          xMin:horizontal,
          xMax:width-horizontal,
          yMin:vertical,
          yMax:height-vertical
        };
      }
    ...
    pointForTool(tool,{
      ...
      const area=Random.reachableAreaForTool(tool,{width,height,extraMargin});

      for(let i=0;i<attempts;i++){
        const point={
          x:Random.number(area.xMin,area.xMax),
          y:Random.number(area.yMin,area.yMax)
        };

        if(existingPoints.every(other=>Utils.distance(point,other)>=minDistance)){
          return point;
        }
      }
    ...
    points(count,{
      ...
      for(let i=0;i<count;i++){
        const p=tool
          ? Random.pointForTool(tool,{
              width,
              height,
              extraMargin,
              existingPoints:result,
              minDistance,
              attempts
            })
          : Random.pointForTool(null,{
              width,
              height,
              extraMargin,
              existingPoints:result,
    ...
    pointOnLine(line,{
      ...
      const area=tool
        ? Random.reachableAreaForTool(tool,{width,height,extraMargin})
        : {xMin:50,xMax:width-50,yMin:50,yMax:height-50};

      for(let i=0;i<attempts;i++){
        const p=line.pointAt(Random.number(-350,350));
        const inside=
          p.x>=area.xMin && p.x<=area.xMax &&
          p.y>=area.yMin && p.y<=area.yMax;

        if(inside && existingPoints.every(other=>Utils.distance(p,other)>=minDistance)){
          return p;
        }
      }

      return {x:line.point.x,y:line.point.y};
    },

    pointAwayFromLine(line,{
      tool=null,
      width=900,
      height=330,
      extraMargin=18,
      ...
      for(let i=0;i<attempts;i++){
        const p=Random.pointForTool(tool,{
          width,
          height,
          extraMargin,
          existingPoints,
        ...
        if(line.distanceToPoint(p)>=minLineDistance){
          return p;
        }
      }

      return Random.pointForTool(tool,{
        width,
        height,
        extraMargin,
        existingPoints,
  ...
  const Check = {
    perpendicularConstruction({
      ...
      if(!baseLine || !point || !drawnLine){
        throw new Error("GeoLib.Check.perpendicularConstruction : paramètres incomplets.");
      }

      const passPoint=drawnLine.distanceToPoint(point)<=pointTolerance;
      const perpendicular=drawnLine.isPerpendicularTo(baseLine,angleTolerance);

      return {
        success:passPoint && perpendicular,
        passPoint,
        perpendicular,
        pointDistance:drawnLine.distanceToPoint(point),
        angleDifference:Math.abs(
          Utils.angleDifference(drawnLine.angle,baseLine.angle)-90
        )
      };
    }
  };
